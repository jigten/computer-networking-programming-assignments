...
static struct pkt a_buff_packet;
...
{
    struct pkt a_packet;
    a_packet.seqnum = expected_seqnum;
    a_buff_packet.seqnum = expected_seqnum;

    a_packet.acknum = expected_acknum;
    a_buff_packet.acknum = expected_acknum;

    int sum = 0, checksum = 0;
    for (int i = 0; i < 20; i++)
    {
        a_packet.payload[i] = message.data[i];
        a_buff_packet.payload[i] = message.data[i];
        sum += (int)message.data[i];
    }
    checksum = ~(sum + a_packet.seqnum + a_packet.acknum);
    checksum++;
    a_packet.checksum = checksum;
    a_buff_packet.checksum = checksum;
    printf("A succesfully sent packet %d\n", a_packet.seqnum);
...
{
    if ((packet.acknum == expected_acknum) && (packet.seqnum + packet.acknum == packet.checksum))
    {
        printf("A succesfully recieved ack packet %d\n", packet.acknum);
        expected_acknum = (expected_acknum + 1) % 2;
        stoptimer(0);
        return;
    }
    else
    {
        printf("A recieved nack packet %d, expected %d\n", packet.acknum, expected_acknum);
        printf("A will retransmit last packet\n");
        tolayer3(0, a_buff_packet);
    }
}
...
void B_input(packet) struct pkt packet;
{
    ...
    for (int i = 0; i < 20; i++)
    {
        recvData[i] = packet.payload[i];
        sum += (int)packet.payload[i];
    }
    checksum = sum + packet.seqnum + packet.acknum + packet.checksum;

    if (packet.seqnum == expected_seqnum && checksum == 0)
    {
        printf("B sucessfully recieved packet with no errors %d\n", packet.seqnum);
        tolayer5(1, recvData);
        b_packet.acknum = expected_seqnum;
        expected_seqnum = (expected_seqnum + 1) % 2;
        b_packet.seqnum = expected_seqnum;
        b_packet.checksum = b_packet.acknum + b_packet.seqnum;
        tolayer3(1, b_packet);
        printf("B sucessfully sent ack packet %d\n", (expected_seqnum + 1) % 2);
    }

    else if (checksum != 0)
    {
        printf("B recieved packet with errors %d\n", packet.seqnum);
        printf("Sending nack\n");
        b_packet.acknum = (expected_seqnum + 1) % 2;
        b_packet.seqnum = packet.seqnum;
        b_packet.checksum = b_packet.acknum + b_packet.seqnum;
        tolayer3(1, b_packet);
        printf("B sucessfully sent nack packet %d\n", (expected_seqnum + 1) % 2);
    }

    else if (packet.seqnum != expected_seqnum)
    {
        printf("B recieved duplicate packet %d\n", packet.seqnum);
        printf("Resending ack\n");
        b_packet.acknum = expected_seqnum;
        b_packet.seqnum = expected_seqnum;
        b_packet.checksum = b_packet.acknum + b_packet.seqnum;
        tolayer3(1, b_packet);
        printf("B sucessfully resent ack packet %d\n", (expected_seqnum + 1) % 2);
    }
}
